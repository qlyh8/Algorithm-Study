 ...
 * 그가 탑승하게 될 우주선은 Alpha Centauri 라는 새로운 인류의 보금자리를 개척하기 위한 대규모 생활 유지 시스템을 탑재하고 있기 때문에,
 ...
 * 김우현은 공간이동 장치 작동시의 에너지 소모가 크다는 점을 잘 알고 있기 때문에 x 지점에서 y 지점을 향해 최소한의 작동 횟수로 이동하려 한다.
 * 하지만 y 지점에 도착해서도 공간 이동장치의 안전성을 위하여 y 지점에 도착하기 바로 직전의 이동거리는 반드시 1광년으로 하려 한다.
 ...
 *
 * [출력]
 * 각 테스트 케이스에 대해 x 지점으로부터 y 지점까지 정확히 도달하는데 필요한 최소한의 공간이동 장치 작동 회수를 출력한다.
 * 3
 * 3
 * 4
 * */
/**
 * 참고1) http://blog.naver.com/PostView.nhn?blogId=occidere&logNo=220982644540&categoryNo=0&parentCategoryNo=0&viewDate=&currentPage=1&postListTopCurrentPage=1&from=postView
 * 참고2) https://zetawiki.com/wiki/BOJ_1011_Fly_me_to_the_Alpha_Centauri
 * 어렵다. 최소 횟수로 규칙 찾기, 노가다..
 *
 * 01: 1					1
 * 02: 1 1				    2
 * 03: 1 1 1				3
 ...
 * 1 2 3 3 4 4 5 5 5  6  6  6  7  7  7  7  8  8  8  8  9
 *
 * int result = (int)(Math.sqrt(distance-0.5)*2.0);
 * */
public class Q75_1011 {
	public static void main(String[] args) throws Exception {
		...
		for(int i=0 ; i<T ; i++){
			...
			int distance = Integer.parseInt(location[1]) - Integer.parseInt(location[0]);
			int result = (int)(Math.sqrt(distance-0.5)*2.0);
